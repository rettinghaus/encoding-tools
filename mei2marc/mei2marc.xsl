...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns="http://www.loc.gov/MARC21/slim" xmlns:mei="http://www.music-encoding.org/ns/mei" xmlns:marc="http://www.loc.gov/MARC21/slim" exclude-result-prefixes="marc mei">
  ...
  <xsl:param name="model_path">http://www.loc.gov/standards/marcxml/schema/MARC21slim.xsd</xsl:param>
                ...
                <xsl:variable name="codeFromList">
                  <xsl:choose>
                    <xsl:when test="$marcMusPerfList/marc:instr[lower-case(.)=lower-case($term)]">
                      <xsl:value-of select="$marcMusPerfList/marc:instr[lower-case(.)=lower-case($term)]/@code" />
                    </xsl:when>
                    <xsl:when test="$iamlMusPerfList/marc:instr[lower-case(.)=lower-case($term)]">
                      <xsl:value-of select="$iamlMusPerfList/marc:instr[lower-case(.)=lower-case($term)]/@code" />
                    </xsl:when>
                    <xsl:otherwise>
        ...
        <xsl:call-template name="subfield">
          ...
          <xsl:with-param name="value">
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:value-of select="$marcLangList/marc:lang[lower-case(.)=lower-case($thisTerm)]/@code" />
              </xsl:otherwise>
            </xsl:choose>
          </xsl:with-param>
        </xsl:call-template>
              ...
              <xsl:with-param name="value">
                ...
                <xsl:choose>
                  ...
                  <xsl:when test="$workCreation/mei:date[mei:date][1]/mei:date[1]/@notbefore">
                    <xsl:value-of select="substring($workCreation/mei:date[mei:date][1]/mei:date[1]/@notbefore,
                      1, 4)" />
                  </xsl:when>
            ...
            <xsl:call-template name="subfield">
              ...
              <xsl:with-param name="value">
                ...
                <xsl:choose>
                  ...
                  <xsl:when test="$workCreation/mei:date[mei:date][1]/mei:date[position()=last()]/@notafter">
                    <xsl:value-of select="substring($workCreation/mei:date[mei:date][1]/mei:date[position()=last()]/@notafter,
                      1, 4)" />
                  </xsl:when>
                  <!-- first date contains multiple date elements, the last of which has content -->
                  <xsl:when test="$workCreation/mei:date[mei:date][1]/mei:date[position()=last()]/text()">
                    <xsl:value-of select="$workCreation/mei:date[mei:date][1]/mei:date[position()=last()]" />
                  </xsl:when>
